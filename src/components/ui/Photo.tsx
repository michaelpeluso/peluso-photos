...
export const Photo = ({ url, alt, label, clickable = true }: PhotoProps) => {
    ...
    return (
        <>
            <div ref={ref} className={`transition-all duration-700 ease-out ${animationClass}`} onClick={clickable ? () => setModalOpen(true) : undefined}>
                {clickable ? (
                    imageContent
                ) : (
            ...
            {/* Modal */}
            {clickable && <Modal url={modalOpen ? url : null} onClose={() => setModalOpen(false)} />}
        </>
    );
};
